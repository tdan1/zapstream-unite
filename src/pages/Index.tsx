...
const Index = () => {
  ...
  const handleWalletConnect = (provider: ethers.providers.Web3Provider, signer: ethers.Signer, address: string) => {
    setWallet({
      connected: true,
      address,
      provider,
  ...
  const handleStakingSuccess = (txHash: string) => {
    ...
    toast({
      ...
      action: <Button variant="outline" size="sm" onClick={() => window.open(`https://optimistic.etherscan.io/tx/${txHash}`, "_blank")} className="flex items-center gap-1">
  ...
  return <div className="min-h-screen flex flex-col relative bg-gradient-to-b from-white to-purple-50 dark:from-gray-950 dark:to-purple-950">
      <WaveBackground />
      ...
      <header className="w-full py-6 relative z-10">
        <div className="container mx-auto px-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <WalletConnect onConnect={handleWalletConnect} onDisconnect={handleWalletDisconnect} connected={wallet.connected} address={wallet.address} assets={wallet.balance} />
          </div>
        </div>
      </header>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <section className="mb-12">
            <Tabs defaultValue="stake" value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="stake" className="animate-scale-in">
                <StakingInterface connected={wallet.connected} signer={wallet.signer} provider={wallet.provider} ethBalance={wallet.balance.eth} swethBalance={wallet.balance.sweth} onSuccess={handleStakingSuccess} />
              </TabsContent>
              
              <TabsContent value="restake" className="animate-scale-in">
                <StakingInterface connected={wallet.connected} signer={wallet.signer} provider={wallet.provider} ethBalance={wallet.balance.eth} swethBalance={wallet.balance.sweth} isRestaking={true} onSuccess={handleStakingSuccess} />
              </TabsContent>
              
              <TabsContent value="swap" className="animate-scale-in">
                <SwapInterface connected={wallet.connected} signer={wallet.signer} provider={wallet.provider} address={wallet.address} onSuccess={handleSwapSuccess} />
              </TabsContent>
            </Tabs>
          </section>

          {/* Recent Transactions */}
          {wallet.connected && transactions.length > 0 && <section className="mb-12 animate-fade-in">
              <h2 className="text-xl font-semibold mb-4 text-purple-800 dark:text-purple-300">Recent Transactions</h2>
              <div className="space-y-3">
                {transactions.slice(0, 5).map(tx => <div key={tx.id} className="transaction-card glass-panel dark:bg-gray-900/60 dark:border-purple-900/30 hover:border-purple-200 dark:hover:border-purple-800/50">
                    <div className="flex justify-between items-center">
                      <div>
                        <div className="flex items-center">
                          {tx.type === TransactionType.SWAP && <div className="flex items-center mr-2">
                          ...
                          {tx.type === TransactionType.STAKE && <div className="flex items-center mr-2">
                              <TokenIcon token={TOKENS.ETH} size={16} className="mr-1" />
                              <ArrowLeftRight className="h-3 w-3 mx-1 text-purple-500" />
                              <TokenIcon token={TOKENS.SWETH} size={16} className="ml-1" />
                            </div>}
                          {tx.type === TransactionType.RESTAKE && <div className="flex items-center mr-2">
                          ...
                          <span className="text-sm font-medium text-purple-800 dark:text-purple-300">
                            {tx.type === TransactionType.STAKE ? "Staked ETH" : tx.type === TransactionType.RESTAKE ? "Restaked swETH" : `Swapped ${tx.fromToken.symbol} to ${tx.toToken.symbol}`}
                          </span>
                          <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${tx.status === TransactionStatus.PENDING ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-200" : tx.status === TransactionStatus.CONFIRMED ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-200" : "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-200"}`}>
                            {tx.status}
                          </span>
                        </div>
                      ...
                      {tx.hash && <Button variant="ghost" size="sm" onClick={() => window.open(`https://optimistic.etherscan.io/tx/${tx.hash}`, "_blank")} className="text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300">
                          <ExternalLink size={14} />
                        </Button>}
                    </div>
                  </div>)}
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
            <div className="glass-panel hover:shadow-lg transition-shadow dark:bg-gray-900/40 dark:border-purple-900/30 group">
              ...
              <Button variant="outline" onClick={() => setActiveTab("stake")} className="w-full border-purple-200 text-purple-600 hover:bg-gradient-to-r hover:from-purple-600 hover:to-blue-600 hover:text-white dark:border-purple-900/30 dark:text-purple-400 relative">
                Start Staking
              </Button>
            </div>
            ...
            <div className="glass-panel hover:shadow-lg transition-shadow dark:bg-gray-900/40 dark:border-purple-900/30">
              ...
              <Button variant="outline" onClick={() => setActiveTab("restake")} className="w-full border-purple-200 text-purple-600 hover:bg-gradient-to-r hover:from-purple-600 hover:to-blue-600 hover:text-white dark:border-purple-900/30 dark:text-purple-400 relative">
                Restake Now
              </Button>
            </div>
            ...
            <div className="glass-panel hover:shadow-lg transition-shadow dark:bg-gray-900/40 dark:border-purple-900/30">
              ...
              <Button variant="outline" onClick={() => setActiveTab("swap")} className="w-full border-purple-200 text-purple-600 hover:bg-gradient-to-r hover:from-purple-600 hover:to-blue-600 hover:text-white dark:border-purple-900/30 dark:text-purple-400 relative">
                Swap Tokens
              </Button>
            </div>
          </section>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="mb-4 md:mb-0">
              <p className="text-sm text-purple-600/60 dark:text-purple-400/60">© 2025 SwellFlow. All rights reserved.</p>
            </div>
